...
### 중심 허브 구조
이 프로젝트는 **모든 기능의 중심 허브** 역할을 합니다.

### Phase 1: 통합 개발
```
domain.com/                    # 메인 허브
├── /tools/json               # JSON 포맷터
├── /tools/variable           # 변수명 생성기
├── /canvas                   # 간단 그림판
├── /chat                     # 간단 채팅방
└── /tools/...               # 기타 도구들
```
...
### Phase 2: 선택적 분리
```
domain.com/                   # 중심 허브 (계속 유지)
├── 간단한 도구들 (내부)
└── 복잡한 기능들 → 외부 분리

diary.domain.com             # AI 일기 (별도 소스)
menu.domain.com              # AI 메뉴추천 (별도 소스)
finance.domain.com           # 가계부 (별도 소스)
```
...
// 헤더에서 모든 기능 접근
<nav>
  <Link href="/tools">간단 도구</Link>
  <Link href="/canvas">그림판</Link>
  <a href="https://diary.domain.com">AI 일기</a>
  <a href="https://menu.domain.com">메뉴 추천</a>
</nav>
```

### 공통 디자인 시스템
- 모든 서브도메인에 동일한 브랜딩 적용
- 일관된 색상/폰트/로고 시스템
- 메인 허브로 돌아가는 네비게이션
