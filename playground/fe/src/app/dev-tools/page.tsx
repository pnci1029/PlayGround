...
import { getDevToolsCategories } from '@/lib/tools-data'
...
export default function DevToolsPage() {
  const devToolsCategories = getDevToolsCategories()
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const savedOpen = localStorage.getItem('devToolsOpenCategories')
        if (savedOpen) {
  ...
  useEffect(() => {
    localStorage.setItem('devToolsOpenCategories', JSON.stringify(openCategories))
  }, [openCategories])
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-8">
        <div className="mb-12">
          ...
          <p className="text-lg text-gray-600 max-w-2xl">
            개발자를 위한 전문 유틸리티 모음입니다. JSON 포매터, 변수명 생성기, 인코딩 도구 등 개발 작업에 필요한 다양한 도구들을 제공합니다.
          </p>
        </div>

        <div className="space-y-6 sm:space-y-8 lg:space-y-10">
          {Object.entries(devToolsCategories).map(([categoryName, tools]) => {
