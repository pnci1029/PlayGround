...
import { getInfoToolsCategories } from '@/lib/tools-data'
...
export default function InfoToolsPage() {
  const infoToolsCategories = getInfoToolsCategories()
  
  const [isHydrated, setIsHydrated] = useState(false)
  const [isLoading, setIsLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [openCategories, setOpenCategories] = useState<string[]>(['정보 도구'])
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const savedOpen = localStorage.getItem('infoToolsOpenCategories')
        if (savedOpen) {
  ...
  useEffect(() => {
    localStorage.setItem('infoToolsOpenCategories', JSON.stringify(openCategories))
  }, [openCategories])
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-8">
        <div className="mb-12">
          ...
          <p className="text-lg text-gray-600 max-w-2xl">
            정보 분석과 트렌드를 확인할 수 있는 도구들입니다. 실시간 트렌드, 데이터 분석 등 다양한 정보 서비스를 제공합니다.
          </p>
        </div>

        <div className="space-y-6 sm:space-y-8 lg:space-y-10">
          {Object.entries(infoToolsCategories).map(([categoryName, tools]) => {
