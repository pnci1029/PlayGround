...
if [[ "$BLOG_CHANGED" == "true" ]]; then
    ...
    docker compose -p blog_service build --no-cache
...
if [[ "$MOODBITE_CHANGED" == "true" ]]; then
    ...
    docker compose -p moodbite_service build --no-cache
...
if [[ "$PLAYGROUND_CHANGED" == "true" ]]; then
    ...
    docker compose -p playground_service build --no-cache
...
if [[ "$TREND_CHANGED" == "true" ]]; then
    ...
    docker compose -p trend_service build --no-cache
...
if [[ "$STOCK_SCREENER_CHANGED" == "true" ]]; then
    ...
    docker compose -p stock_screener_service build --no-cache
...
if [[ "$DOCKER_CHANGED" == "true" ]]; then
    ...
    for project in playground moodbite trend blog stock_screener; do
        if [ -d "$project" ] && [ -f "$project/docker-compose.yml" ]; then
            ...
            docker compose -p ${project}_service build --no-cache
