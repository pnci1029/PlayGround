...
### P1. 편집 기능 — AI 글 수정 + 직접 작성  → [roadmap-editing.md](roadmap-editing.md)  🚧 진행 중(2026-06-25)
...
- 작업량: 중간. 사용자 통제력↑. → 상세 구현계획은 roadmap-editing.md "구현 계획" 절.

### P2. 생성 대기 UX 개선  → [roadmap-generation-ux.md](roadmap-generation-ux.md)  🚧 진행 중(2026-06-25)
- **문제**: 자기검수 추가로 생성이 ~1분(아웃라인+산문+검수 3콜)인데 스피너만 → 답답.
- **옵션 A(가벼움, 우선)**: 진행 단계 표시 — "구상 중 → 본문 쓰는 중 → 다듬는 중". 순수 FE 추정 타이머 + 진행바.
- **옵션 B(중간)**: SSE 스트리밍 — 본문이 실시간으로 써지는 연출. BE SSE 라우트 + FE 스트림 처리. (추후)
- 추천: A → 이후 B. **이번엔 A 구현.**
